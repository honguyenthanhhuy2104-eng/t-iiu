...
if not TOKEN:
    print("❌ Missing TOKEN (set trong Railway Variables)")
    exit()
...
async def safe_edit(msg, content=None, embed=None):
    try:
...
async def pay(ctx,member:discord.Member,amount:int):
    ...
    await ctx.send("✅ chuyển thành công")
...
async def flip(ctx,bet,choice:str):
    ...
    for _ in range(5):
        await safe_edit(msg, content=random.choice(["🪙","💿"]))
...
async def slot(ctx,bet):
    ...
    for _ in range(6):
        await safe_edit(msg, content=" | ".join(random.choices(icons,k=3)))
        await asyncio.sleep(0.12)
...
async def taixiu(ctx,bet,choice:str):
    ...
    for i in range(8):
        await safe_edit(msg, content=" ".join(random.choices(emoji,k=3)))
        await asyncio.sleep(0.1+i*0.01)
...
async def baucu(ctx,bet,choice:str):
    ...
    for _ in range(8):
        ...
        await safe_edit(msg, content=" | ".join(fake))
        await asyncio.sleep(0.12)
